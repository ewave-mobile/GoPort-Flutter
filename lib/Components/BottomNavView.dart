...
class BottomNavView extends StatelessWidget {
  ...
  Widget _buildMenuItem(BuildContext context,BottomNavViewItem item) {
    return InkWell(
      ...
      child: Column(
        children: [
          Image.asset("assets/images/" + item.image, width: 20, height: 20, color: colorGray, matchTextDirection: true,),
          SizedBox(
            height: 8,
          ),
          Text(AppLocalizations.of(context).translate(item.title), style: TextStyle(color: colorGray, fontSize: 12)),
        ],
      ),
    );
  }
